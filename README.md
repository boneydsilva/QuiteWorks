...
— no npm, no framework. What is in `public/` is exactly what gets uploaded.

It is published in **English and eleven Indian languages**, with a page for
**every state and union territory** — 125 pages in all. Five are written by
hand; the other 120 are generated by `python tools\build_i18n.py`, which writes
plain static HTML and nothing else. Run it after changing anything under
`tools/content/`.
...
public/             EVERYTHING IN HERE IS PUBLISHED. Nothing outside it is.
  ...
  hi/ mr/ bn/ ta/   GENERATED. The same four pages in eleven languages.
  te/ kn/ ml/ gu/
  pa/ or/ as/
  india/            GENERATED. The hub plus 36 states and union territories.
  <lang>/india/     GENERATED. Each state in its own language, where they differ.
  robots.txt        GENERATED. Search engine rules
  sitemap.xml       GENERATED. All 124 indexable URLs, with hreflang
  assets/
...
tools/
  build_i18n.py     Writes every generated page. --check fails if public/ is stale.
  content/          The language table, the state data, and the copy for each
  audit_seo.py      Checks all 125 pages without a browser
  check_console.py  Loads a page of every kind in Chrome, asserts zero console output
deploy/
...
3. **Add it to the navigation.** Twice: search for `nav-links` in the five
   hand-written English files, and add it to `header()` and `footer()` in
   `tools\build_i18n.py` so the generated pages get it too. Link to
   `/yournewtool`, not `/yournewtool.html` — the site uses clean URLs.

4. **The sitemap builds itself** from `tools\build_i18n.py`. Add the page
   there rather than editing `public/sitemap.xml`, which is generated.

5. **Add its pricing** as a fourth card in `public/pricing.html`, or give it its own
   pricing page if the model is different.

The header and footer are now in two places — the hand-written English pages
and the builder. Once there are four or five tools, the honest move is to let
`build_i18n.py` generate the English pages too, so there is one copy of the
